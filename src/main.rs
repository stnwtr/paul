...
async fn handle_interaction(interaction: Interaction) {
    println!("In interaction !")
}
...
async fn handler(header_map: HeaderMap, body: String) -> Response {
    ...
    if verify(signature, timestamp, &body) {
        if interaction.r#type == 1 {
            return r#"{"type": 1}"#.into_response();
        }

        tokio::spawn(async move { handle_interaction(interaction).await });
        
        let mut response = StatusCode::ACCEPTED.into_response();
        response.headers_mut().insert("Content-Type", "application/json".try_into().unwrap());
        response
    } else {
